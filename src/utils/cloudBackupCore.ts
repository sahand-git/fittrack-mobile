...
export interface FitnessBackup { version: '2.0'; exportedAt: string; profile: UserProfile; dailyLogs: Record<string, DayLog>; customFoods: FoodItem[]; weightHistory: WeightEntry[]; settings?: { supplementRoutine?: Array<{name:string;category:'vitamin'|'mineral'|'supplement'|'other';amount:number;unit:string;dosage:string;icon:string;part:'morning'|'noon'|'evening'|'bedtime'}> } }
...
const nutrients={calories:num,protein:num,carbs:num,fat:num,fiber:optional(num),sugars:optional(num),sodium:optional(num),vitaminA:optional(num),vitaminC:optional(num),vitaminD:optional(num),vitaminB12:optional(num),calcium:optional(num),iron:optional(num),potassium:optional(num),magnesium:optional(num),zinc:optional(num)};
...
const supplement=shape({id:str,name:str,category:choice('vitamin','mineral','supplement','other'),dosage:str,icon:optional(str),loggedAt:str,part:optional(choice('morning','noon','evening','bedtime')),amount:optional(num),unit:optional(str),taken:optional(bool)});
...
const day=shape({date,meals:shape({breakfast:list(meal),lunch:list(meal),dinner:list(meal),snack:list(meal)}),waterMl:num,steps:num,stepCaloriesBurned:num,workouts:list(workout),supplements:optional(list(supplement)),notes:optional(str),aiReport:optional(report)});
const profile=shape({name:str,email:str,avatarUrl:optional(str),isGoogleConnected:bool,gender:choice('male','female','other'),age:num,heightCm:num,weightKg:num,targetWeightKg:num,activityLevel:choice('sedentary','light','moderate','very_active','athlete'),goal:choice('fat_loss_aggressive','fat_loss_moderate','maintenance','lean_bulk','muscle_gain'),includeStepsInCalorieBudget:bool,stepGoal:num,waterGoalMl:num,bmr:num,tdee:num,targetCalories:num,targetProtein:num,targetCarbs:num,targetFat:num,profileCompleted:bool,onboardingVersion:optional(num),customMacroSplit:optional(shape({proteinPercent:num,carbsPercent:num,fatPercent:num})),supplementSchedule:optional(shape({enabled:bool,frequency:choice('daily_morning','twice_daily','interval_4h','interval_6h','interval_8h','interval_12h'),preferredTime:optional(str)}))});
const settings=shape({supplementRoutine:optional(list(shape({name:str,category:choice('vitamin','mineral','supplement','other'),amount:num,unit:str,dosage:str,icon:str,part:choice('morning','noon','evening','bedtime')})))});
const backup=shape({settings:optional(settings),version:choice('2.0'),exportedAt:str,profile,dailyLogs:v=>{
