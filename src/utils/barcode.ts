...
const fields = 'product_name,product_name_en,product_name_ar,generic_name,brands,nutriments,serving_size,serving_quantity,serving_quantity_unit,nutrition_grades,image_url,ingredients_text,ecoscore_grade,nova_group';
...
const parseMg = (val?: number, unit?: string): number | undefined => {
  if (val === undefined) return undefined;
  const u = (unit || '').toLowerCase();
  if (u === 'g') return round(val * 1000);
  if (u === 'mcg' || u === 'µg') return round(val / 1000);
  if (val < 1 && (!u || u === 'g')) return round(val * 1000);
  return round(val);
};

const parseMcg = (val?: number, unit?: string): number | undefined => {
  if (val === undefined) return undefined;
  const u = (unit || '').toLowerCase();
  if (u === 'mg') return round(val * 1000);
  if (u === 'g') return round(val * 1000000);
  if (val < 0.01 && (!u || u === 'g')) return round(val * 1000000);
  return round(val);
};
...
export function normalizeProduct(barcode: string, product: Record<string, any>): FoodItem {
  ...
  const name = text(product.product_name) || text(product.product_name_en) || text(product.product_name_ar) || text(product.generic_name);
  ...
  const rawVitA = number(n['vitamin-a_100g'] ?? n['vitamin-a_value']);
  const rawVitC = number(n['vitamin-c_100g'] ?? n['vitamin-c_value']);
  const rawVitD = number(n['vitamin-d_100g'] ?? n['vitamin-d_value']);
  const rawVitB12 = number(n['vitamin-b12_100g'] ?? n['vitamin-b12_value']);
  const rawCalcium = number(n['calcium_100g'] ?? n['calcium_value']);
  const rawIron = number(n['iron_100g'] ?? n['iron_value']);
  const rawPotassium = number(n['potassium_100g'] ?? n['potassium_value']);
  const rawMagnesium = number(n['magnesium_100g'] ?? n['magnesium_value']);
  const rawZinc = number(n['zinc_100g'] ?? n['zinc_value']);
  ...
  return {
    ...
    vitaminA: scale(parseMcg(rawVitA, n['vitamin-a_unit'])),
    vitaminC: scale(parseMg(rawVitC, n['vitamin-c_unit'])),
    vitaminD: scale(parseMcg(rawVitD, n['vitamin-d_unit'])),
    vitaminB12: scale(parseMcg(rawVitB12, n['vitamin-b12_unit'])),
    calcium: scale(parseMg(rawCalcium, n['calcium_unit'])),
    iron: scale(parseMg(rawIron, n['iron_unit'])),
    potassium: scale(parseMg(rawPotassium, n['potassium_unit'])),
    magnesium: scale(parseMg(rawMagnesium, n['magnesium_unit'])),
    zinc: scale(parseMg(rawZinc, n['zinc_unit'])),
...
export async function lookupBarcode(raw: string, saved: FoodItem[] = [], transport: Transport = {}): Promise<FoodItem> {
  ...
  if (!/^\d{6,14}$/.test(code)) throw new Error('Enter an 8, 12, 13 or 14 digit barcode number from the package.');
